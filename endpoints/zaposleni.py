...
def third_highest_salary():
	...
	cursor.close() 
	connection.close()
	return jsonify({"third_highest": result[0]}), 200

@zaposleni_bp.route("/like_a")
def like_a():
	connection = get_db_connection()
	cursor = connection.cursor()
	query = "SELECT firstname, lastname, username, email FROM employee where firstname LIKE \"a%\""
	cursor.execute(query)
	result = cursor.fetchall()
	list = []
	for row in result:
		e = {"firstname": row[0], "lastname": row[1], "username": row[2], "email": row[3]}
		list.append(e)
	return list, 200